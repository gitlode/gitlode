...
async function resolveRepositoryBasics(
  ...
  if (explicitRepoUrl !== undefined) {
    ...
    repoUrl = await gitAdapter.getRemoteUrl(repoPath);
  }

  let repoName: string;
  if (explicitRepoName !== undefined) {
    repoName = explicitRepoName;
  } else {
    repoName = deriveRepoName(repoUrl, repoPath);
  }
  return {
    repoName,
    repoUrl,
...
async function buildCustomProjector(
  ...
  progressRuntime: ReturnType<typeof createProgressRuntime>,
  ...
  const pluginInitResults = await initializePlugins(pluginEntries, (entry) => ({
    warn(message) {
      progressRuntime.presenter.renderDiagnostic("warn", `Plugin "${entry.namespace}": ${message}`);
    },
    error(message) {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    /**
     * undefined when not profiling, otherwise the root profiler itself.
     */
    ...
    try {
      ...
      const { repoName: resolvedRepoName, repoUrl: resolvedRepoUrl } =
        await resolveRepositoryBasics(
          resolvedRepoPath,
          gitAdapter,
      ...
      if (!configPath || !hasEffectiveExtensionsConfig(extensions)) {
        ...
        if (projectorResult.kind === "termination") {
          progressRuntime.presenter.renderUserError(projectorResult.message);
          process.exitCode = 1;
          return;
        }

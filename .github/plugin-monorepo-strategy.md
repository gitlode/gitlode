...
| Compatibility declaration      | Official plugins must declare `peerDependencies` on `gitlode`.             | Decided | Keep minor-bounded ranges and update on API boundary changes. See [Plugin Package Policy](../packages/gitlode/docs/design/plugins.md#plugin-package-policy) for the normative peer-range form. |
| Runtime compatibility behavior | Version mismatch should emit warning, not fatal by default.                | Decided | Revisit only if frequent unsafe mismatches occur. Runtime check implemented in CLI layer (`src/cli/plugins.ts`); see [Plugin Package Policy](../packages/gitlode/docs/design/plugins.md#plugin-package-policy) for all four warning cases.                                |
